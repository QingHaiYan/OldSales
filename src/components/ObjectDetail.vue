  ...
  <div class="bj-color" >
      ...
      <el-row>
        <el-col :xs="{span: 24, offset: 0}" :sm="{span: 24, offset: 0}" :md="{span: 24, offset: 0}" :lg="{span: 18, offset: 3}" :xl="{span: 12, offset: 6}">
          <div class="detail-content" ref="bodys">
            <div>
              <el-breadcrumb separator-class="el-icon-arrow-right">
                ...
                <el-breadcrumb-item ><a v-if="this.$route.params.num==28" :href="show_data.url[2].http_url" style="text-decoration:underline;color:blue">{{title}}</a><a v-if="this.$route.params.num!=28">{{title}}</a></el-breadcrumb-item>
              </el-breadcrumb> 
              <!-- http://3d.720smart.com/project/tourdbe7a02e/index.html -->
            </div>
            <div class="margin-top-sm object-bg flex-row main-justify-center cross-align-center position-re">
              <div class="flex-row main-justify-center cross-align-center" ref="banner_touch">
                <img  class="detail-img" :src="big_img" alt=""  @click="clickImg($event)">
              </div>
          ...
          <!-- ================================================================================ -->
              <div class="margin-top-sm Toroll-div" ref="Topack">
                  <div  class="Topack" v-for="(item,index) in show_data.img" :key="index" @click="selectImage(index)"> 
                    <img  :src="item" alt="" width="75%" >
                  </div> 
              </div>
              <!-- 
                <div class="pack-div padding-b-sm" ref="pack">
                  <div  class="roll-div flex-row">
                    <div  class="w-20" v-for="item in all_arr" :key="item.id" @click="jumpObject(item.id)">
               ...
               -->
              <p class=" flex-row main-justify-between">
                <span class="flex-row margin-r-sm">
                <img src="../../static/img/detail/move-left.png" alt="" @click="TomoveLeft()">
                <img src="../../static/img/detail/move-right.png" alt="" @click="TomoveRight()">
                </span> 
              </p>  
            <!-- ================================================================================ -->
              ...
              <div ref="bodys3">
                  ...
                  <transition  v-if="showImg" name="fade">
                    <div  class="img-view" @click="bigImg">
                      <!-- 遮罩层 -->
                      <div class="img-layer">                 
                        <img class="relativePositioning" :src="imgSrc"  >
                      </div>
                      </div>
                  </transition>  
              </div> 
            ...
            <div class="margin-top-sm file"> 
              <div class="framework">
                <p v-if="this.$route.params.num==20 || this.$route.params.num==21 || this.$route.params.num==23"><button class="button margin-b-sm" >平台架构</button></p>
                  <img v-if="this.$route.params.num==20" src="../../static/img/honour/医卓通平台架构图.png" alt="">
                  <img v-if="this.$route.params.num==23 " src="../../static/img/honour/易卓通-水务安全平台架构图.png" alt="">
                  <img v-if="this.$route.params.num==21 " src="../../static/img/honour/易卓通--智慧城市平台架构图.png" alt="">
                  <p>&nbsp;</p>
              </div>  
              <div v-if="this.$route.params.num==20 || this.$route.params.num==21 || this.$route.params.num==23">
                 <p><button class="button margin-b-sm" >平台应用价值</button></p>
                 <ul>
                    <li v-for="(item,index) in show_data.terr_content" :key="index">
                      <p>&nbsp;</p>
                      <p>{{index+1}}>{{item.title}}</p> 
                      <p>&nbsp;</p>
                      <p style="line-height:40px">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{item.content}}</p> 
                    </li> 
                </ul>
                <p>&nbsp;</p>
              </div>
...
export default {
    ...
    return {
       ...
       show_data:{
          ...
          'url':[
              ...
              {
                'http_url':'http://slyybukai.fxbvr.com/'
              }
            ]
       },
       object_detail:{
  ...
  methods:{
        ...
        if(this.object_detail[type]){
          for (let i = 0; i < arr.length; i++) {
            if(type == arr[i].type){
                ...
                // console.log(this.show_data.url[2].http_url);
    ...
    moveLeft(){
     // let tt = this.$refs.pack.scrollLeft;
    ...
    TomoveLeft(){
      //et tt = this.$refs.Topack.scrollLeft;
      clearInterval(this.right_num);
      clearInterval(this.left_num);
      this.left_num = setInterval(()=>{
             this.$refs.Topack.scrollLeft-=3; 
             //console.log(this.$refs.Topack.scrollLeft);
             if(this.$refs.Topack.scrollLeft<=0){
               clearInterval(this.left_num);
             }
        },20)
    },
    TomoveRight(){   
      let tt = this.$refs.Topack.scrollLeft;
      clearInterval(this.right_num);
      clearInterval(this.left_num);
     // console.log(this.$refs.Topack.scrollWidth-this.$refs.Topack.offsetWidth);
      var overflow_num = this.$refs.Topack.scrollWidth-this.$refs.Topack.offsetWidth;
     // console.log(overflow_num);
      this.right_num = setInterval(()=>{
            this.$refs.Topack.scrollLeft+=3; 
            // console.log(this.$refs.Topack.scrollLeft);
            if(this.$refs.Topack.scrollLeft>=overflow_num || this.$refs.Topack.scrollLeft-tt>200){
               clearInterval(this.right_num);
            }
      },10)
    },
    ...
    getAngle(){
       if (Publics.getType().android ||Publics.getType().ios ||Publics.getType().iPad) { //判断设备
            ...
            var banner_touch2 = this.$refs.bodys3;
              // Publics.touchEvent(banner_touch2,true,(num)=>{  //大图的左右滑动方法
              //           this.judgeType(num); 
              //   });
              // console.log(num);
     ...
     getData(){
        ...
        .then(response=>{
              if(response.data.status == 200){
                ...
                // console.log(this.object_detail);
                this.show_data = this.object_detail.big_screen;
                this.big_img = this.show_data.img[0];
                this.terr_content = this.show_data.terr_content;
              //  console.log(this.terr_content);
              }else{
      ...
      getInfoList(){
        ...
        .then(response=>{
              if(response.data.status == 200){
                ...
                this.newObject_data1 = this.object_data[0];
                this.newObject_data2 = this.object_data[1];
                this.newObject_data3 = this.object_data[2];
                this.newObject_data4 = this.object_data[3];
                this.all_arr = this.newObject_data1.concat(this.newObject_data2).concat(this.newObject_data3).concat(this.newObject_data4)
